...
import static ug.karuhanga.planimeter.GLUseful.gl;
import static ug.karuhanga.planimeter.GLUseful.gl;
...
public class GeomBuilder
  /*
    ...
    colour. Then call Add to add vertex definitions (using class ug.karuhanga.planimeter.Vec3f
    to define points, and ug.karuhanga.planimeter.GeomBuilder.Color to define colours), and
    use the returned vertex indices to construct faces with AddTri and
    AddQuad. Finally, call MakeObj to obtain a ug.karuhanga.planimeter.GeomBuilder.Obj that
  ...
  */
  {
    ...
    public GeomBuilder
      ...
      )
      {
        ...
        BoundMin = null;
        BoundMax = null;
      } /*ug.karuhanga.planimeter.GeomBuilder*/
    ...
    private int AddActual
      ...
      )
      {
        if (!AutoNormals)
          {
            throw new RuntimeException("ug.karuhanga.planimeter.GeomBuilder.AddActual shouldn’t be called if not AutoNormals");
          } /*if*/
        return
            Add
              (
    ...
    private void GenAutoNormal
      ...
      )
      {
        if (!AutoNormals)
          {
            throw new RuntimeException("ug.karuhanga.planimeter.GeomBuilder.GenAutoNormal shouldn’t be called if not AutoNormals");
          } /*if*/
        final Vec3f
    ...
    public Obj MakeObj()
      /* constructs and returns the final geometry ready for rendering. */
      {
        if (Points.size() == 0)
          {
            throw new RuntimeException("ug.karuhanga.planimeter.GeomBuilder: empty object");
          } /*if*/
